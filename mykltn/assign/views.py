...
def assign(request):
    my_lecturer_list = Lecturer.objects.filter(degree_id__lte=3)
    my_group_list = MyGroup.objects.filter(id__gt=1)
...
def LecturerList(request):
    lecturer_list = Lecturer.objects.all().order_by("-create_date")
    context = {
...
def create_topic(request):
    ...
    topic = Topic.objects.create(title=topic_name, student=student_name, mssv=student_id,mentor_id=mentor_id)
    topic.save()
    for keyword in keyword_list:
        keyword_level = request.POST["keyword_" + str(keyword.id) + "_level"]
...
def review_assign(request):
    ...
    my_lecturer_list = Lecturer.objects.filter(degree_id__lte=3).order_by("id")
    for lec in my_lecturer_list:
        lec.review_count=0
        lec.save()
